...
def main():
    ...
    for episode in range(EPISODE):
        ...
        task = tg.ACWTask(metatrain_character_folders, CLASS_NUM, SAMPLE_NUM_PER_CLASS, split="train", remove=REMOVE_CLASS)
        ...
        sample_features = sample_features.view(task.num_classes, SAMPLE_NUM_PER_CLASS, 64, 1, 89)
        # print('sample_features.shape', sample_features.shape)
        ...
        if (episode + 1) % 5 == 0:
            ...
            for i in range(TEST_EPISODE):
                task = tg.ACWTask(metatest_character_folders, CLASS_NUM, SAMPLE_NUM_PER_CLASS, split="test", remove=REMOVE_CLASS)
